---
title: "Fitting genotype by environment models in pedigreemm"
author: "Giovanny Covarrubias-Pazaran & Ana Inez Vazquez"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{GxE models in pedigreemm}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

The purpose of this vignette is to show how to fit different genotype by environment (GxE) models using the pedigreemm package:  
...
ansMain <- pedigreemm(Yield ~ Env + (1|Name),
...
ansDG <- pedigreemm(as.formula(diagFormula),
                      pedigree = list(Name = Matrix::chol(A) ),
                      data=DT)
...
```{r}
Z <- with(DT, dsc(Env))$Z
diagFormula <- paste0( "Yield ~ Env + (", paste(colnames(Z), collapse = "+"), "|| Name)")
for(i in 1:ncol(Z)){DT[,colnames(Z)[i]] <- Z[,i]}
ansCSDG <- pedigreemm(as.formula(diagFormula),
                      pedigree = list(Name = Matrix::chol(A) ),
                      data=DT)
...
H0 <- ans1a$uList$`vsc(dsc(Env), isc(Name))` # GxE table
# reduced rank model
ans1b <- mmec(y~Env,
              random=~vsc( dsc(rrc(Env, H = H0, nPC = 3)) , isc(Name)) + # rr
...
## get the loadings
loadings=with(DT, rrc(Env, nPC = 3, H = wide0, returnGamma = T) )$Gamma
## extract factor scores from the model
scores <- ans1b$uList[[1]];
## BLUPs for all environments
E= scores %*% t(loadings)  
## Extract the covariance matrix
vc <- ans1b$theta[[1]];vc # rr
vc2 <- ans1b$theta[[2]];vc2 # diagonal
G = loadings %*% vc %*% t(loadings) + vc2 
