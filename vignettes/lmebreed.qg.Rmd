...
9) Bivariate models with repeated records
...
In the previous example we showed how the left side of the formulae to specify the covariance structure and the effect on the right side `(covStructure | effect)` in the `lmeb()` solver. By specifying `( CA.2011+CA.2012+CA.2013 || Name)` we declare a covariance structure for Name between environments. The `||` indicates that we do not want to fit covariance between these environments This is considered a CS + DIAG (compound symmetry + diagonal) model.
...
and takes advantage of the variance covariance matrix for the genotype effect known as the additive relationship matrix (A) and calculated using the `A.matr` function to establish connections among all individuals and predict the BLUPs for individuals that were not measured. The prediction accuracy depends on several factors such as the heritability ($h^2$), training population used (TP), size of TP, etc.
...
### 10) Bivariate model with repeated records
...
# 
# Z <- with(DTC, smm(leg(X,1)) )
# for(i in 1:ncol(Z)){DTC[,colnames(Z)[i]] <- Z[,i]}
# 
# X <- with(DTC, smm(TRAIT) )
...
# 
# ## 
# mRR2b<-lmeb(formula = Y ~ Xf + 
#               (0 + T1:leg0 + T1:leg1 + T2:leg0 + T2:leg1 | SUBJECT) + 
#               (0 + T1 + T2 || unitsR),
#             relmat = list(SUBJECT=A), nIters = 1000, verbose = FALSE,
#             data=DTC)
...
As can be seen, the two models are equivalent. The lme4breeding library uses this trick directly by setting the argument rotation=TRUE. Remember this trick only works for balanced datasets.
