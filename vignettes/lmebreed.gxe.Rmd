...
with(DTc, plot(gebv,gv, col=cols))
...
lmod <- lmebreed(formula=as.formula(csdiagFormula), data = DT, 
                  ...
                  #   # calc.derivs = FALSE,
                  #   # restart_edge = FALSE,
                  #   optCtrl = list(maxfun = 2500, maxeval = 2500)
                  # ),
...
For the sake of time we are not running this model. Also because the heterogeneous residual model does not increase the accuracy in this particular example.

Notice that we have fitted an MET model with 1K individuals in 50 environments, with 2 replicates in each environment with a genomic relationship matrix in a few seconds. It hardly gets any better than this.
